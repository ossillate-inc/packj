...
from packj.util.net import (
    __parse_url,
    download_file,
    check_site_exist,
    check_domain_popular,
)
...
def msg_ok(x):
    if len(x) > 50:
        x = ''.join(x[0:46]) + ' ...'
    msg_info(
        f'{Style.BRIGHT}{Fore.GREEN}PASS{Style.RESET_ALL} [{Fore.BLUE}{x}{Style.RESET_ALL}]'
    )
...
def msg_warn(tag, x):
    msg_info(
        f'{Style.BRIGHT}{Fore.YELLOW}{tag}{Style.RESET_ALL} [{Fore.MAGENTA}{x}{Style.RESET_ALL}]'
    )
...
def build_threat_model(filename):
    try:
        ...
        if (
            'audit' in config_data
            and 'alerts' in config_data['audit']
            and config_data['audit']['alerts']
        ):
...
def analyze_yanked_releases(
    pm_proxy, pkg_name, pkg_info, risks, report, release_history
):
    try:
        ...
        if (num_releases == num_yanked) or (
            num_releases > 3 and num_yanked > int(num_releases / 2)
        ):
            reason = (
                f'more than 50% ({num_yanked}) of {num_releases} releases were yanked'
            )
...
def analyze_release_history(
    pm_proxy, pkg_name, pkg_info, risks, report, release_history=None
):
    try:
...
def analyze_release_time(
    pm_proxy, pkg_name, ver_str, pkg_info, risks, report, release_history=None
):
    try:
...
def analyze_deps(pm_proxy, pkg_name, ver_str, pkg_info, ver_info, risks, report):
    try:
        ...
        deps = pm_proxy.get_dependencies(
            pkg_name, ver_str=ver_str, pkg_info=pkg_info, ver_info=ver_info
        )
...
def analyze_empty_catchblock(pm_name, pkg_name, pkg_info, filepath, risks, report):
    msg_info(
        'Checking for sensitive API calls in empty catch blocks', end='', flush=True
    )
    ...
    try:
        if pm_name == 'pypi' or pm_name == 'local_python':
            ...
            catch_clauses = _get_api_declrs(filepath)['EmptyCatchClauses']
                ...
                for clause in catch_clauses:
                    for func in api_results:
                        if pm_name == 'pypi':
                            ...
                            clause_filename = clause['File'].replace(pkg_name, '')
                        func_filename = func['range']['start']['fileInfo']['file']
                        if clause_filename == func_filename:
                            if (
                                clause['StartLine']
                                < func['range']['start']['row']
                                < clause['EndLine']
                            ):
                                results['count'] += 1
                                results['results'].append(
                                    f'{func_filename}:{clause["StartLine"]}-{clause["EndLine"]}'
                                )
            if results['count'] > 0:
                msg_alert(
                    f'found {results["count"]} empty catch caluse(s) with sensitive API call(s)'
                )
            else:
                msg_ok('No senstive calls in empty catch clauses found')
            report['sensitive_calls_empty_catch_clause'] = results['results']
        else:
            msg_warn('N/A', 'Coming soon!')
    except Exception as e:
        msg_fail(str(e))
...
def analyze_repo_activity(risks, report):
    try:
        ...
        if reason:
            ...
            commits, contributors, tags = tuple(
                repo_data[k] for k in ('commits', 'contributors', 'tags')
            )
...
def analyze_repo_url(pm_proxy, pkg_name, ver_str, pkg_info, ver_info, risks, report):
    try:
        ...
        repo_url = pm_proxy.get_repo(
            pkg_name, ver_str=ver_str, pkg_info=pkg_info, ver_info=ver_info
        )
        if not repo_url:
            repo_url = pm_proxy.get_homepage(
                pkg_name, ver_str=ver_str, pkg_info=pkg_info
            )
            if not repo_url or not repo_url.startswith(popular_hosting_services):
                repo_url = None
        if not repo_url:
            repo_url = pm_proxy.get_download_url(
                pkg_name, ver_str=ver_str, pkg_info=pkg_info
            )
            if not repo_url or not repo_url.startswith(popular_hosting_services):
                repo_url = None
...
def analyze_composition(pm_name, pkg_name, ver_str, filepath, risks, report):
    ...
    try:
        ...
        if num_funcs == 0 or (
            not report.get('permissions', None)
            and not report.get('dependencies', None)
            and num_funcs <= 5
            and lang_files <= 1
        ):
            ...
            msg_ok(
                f'{num_funcs} funcs across {lang_files} {lang_file_ext} files ({total_loc} Loc)'
            )
    except Exception as e:
        msg_fail(str(e))
    finally:
...
ALERTS = {
    ...
    'SOURCE_SETTINGS': Alert(
        Risk.ENV_READ, 'reads system settings or environment variables'
    ),
    'SINK_UNCLASSIFIED': Alert(
        Risk.ENV_WRITE, 'modifies system settings or environment variables'
    ),
    'SOURCE_ACCOUNT': Alert(
        Risk.ENV_WRITE, 'modifies system settings or environment variables'
    ),
...
def trace_installation(pm_enum, pkg_name, ver_str, report_dir, risks, report):
    try:
        ...
        stdout, stderr, error = exec_command(
            "strace", check_strace_cmd, redirect_mask=3
        )
        if error:
        ...
        _, trace_filepath = tempfile.mkstemp(
            prefix='trace_', dir=report_dir, suffix='.log'
        )

        strace_cmd = f'{strace_bin} -f -e trace=network,file,process -ttt -T -o {trace_filepath } {install_cmd}'
        stdout, stderr, error = exec_command(
            "strace", strace_cmd.split(), redirect_mask=3
        )
        if error:
...
def audit(pm_args, pkg_name, ver_str, report_dir, extra_args, config):
    ...
    msg_info('===============================================')
    msg_info(
        f'Auditing {pm_name} package {pkg_name} (ver: {ver_str if ver_str else "latest"})'
    )
    msg_info('===============================================')
    ...
    try:
        ...
        pkg_name, pkg_info = pm_proxy.get_metadata(
            pkg_name=pkg_name, pkg_version=ver_str
        )
    ...
    # analyze metadata
    risks, report = analyze_pkg_descr(
        pm_proxy, pkg_name, ver_str, pkg_info, risks, report
    )
    risks, report, release_history = analyze_release_history(
        pm_proxy, pkg_name, pkg_info, risks, report
    )
    risks, report = analyze_yanked_releases(
        pm_proxy, pkg_name, pkg_info, risks, report, release_history
    )
    risks, report = analyze_version(pm_proxy, pkg_name, ver_info, risks, report)
    risks, report = analyze_release_time(
        pm_proxy, pkg_name, ver_str, pkg_info, risks, report, release_history
    )
    risks, report = analyze_author(
        pm_proxy, pkg_name, ver_str, pkg_info, ver_info, risks, report
    )
    risks, report = analyze_readme(pm_proxy, pkg_name, ver_str, pkg_info, risks, report)
    # risks, report = analyze_homepage(
    #    pm_proxy, pkg_name, ver_str, pkg_info, risks, report
    # )
    risks, report = analyze_downloads(pm_proxy, pkg_name, pkg_info, risks, report)
    risks, report = analyze_zero_width_unicode(
        pm_proxy, pkg_name, pkg_info, risks, report
    )
    ...
    risks, report = analyze_repo_url(
        pm_proxy, pkg_name, ver_str, pkg_info, ver_info, risks, report
    )
    if 'repo' in report and 'url' in report['repo'] and report['repo']['url'] and False:
    ...
    if not os.path.isdir(pkg_name):
        try:
            msg_info(f"Downloading package from {pm_name}...", end='', flush=True)
    ...
    if filepath:
        ...
        risks, report = analyze_empty_catchblock(
            pm_name, pkg_name, pkg_info, filepath, risks, report
        )

    # perform dynamic analysis if requested
    if install_trace:
        risks, report = trace_installation(
            pm_enum, pkg_name, ver_str, report_dir, risks, report
        )
    ...
    if pm_enum == PackageManagerEnum.pypi:
        msg_info(
            f'=> View pre-vetted package report at https://packj.dev/package/PyPi/{pkg_name}/{ver_str}'
        )
    return report
...
def parse_request_args(args):
    ...
    if in_docker() or in_podman():
        ...
        if not host_volume or not os.path.exists(container_mountpoint):
            msg_info(
                f'Missing host volume at {container_mountpoint}. Run Docker/Podman with "-v /tmp:{container_mountpoint}" argument.'
            )
            exit(1)
    ...
    for item in args.depfiles:
        try:
            assert (
                ':' in item
            ), f'invalid dep file: {item}. Expected <pm>:<file> (e.g., npm:package.json)'
            ...
            msg_info(
                f'Failed to parse file "{item}" for dependencies: {str(e)}. Ignoring'
            )
    ...
    for item in args.packages:
        try:
            ...
            assert (
                len(components) >= 2 and len(components) <= 3
            ), f'Invalid request: {item}. Expected <pm>:<pkg>[:<ver>] (e.g., npm:react)'
    ...
    if args.debug:
        try:
            _, filename = tempfile.mkstemp(
                prefix='debug_', dir=report_dir, suffix='.log'
            )
    ...
    if args.trace:
        if not (in_docker() or in_podman()):
            print(
                f'*** You\'ve requested package installation trace *** We recommend running in Docker/Podman. Continue (N/y): ',
                end='',
            )
            stop = input()
            if stop != 'y':
    ...
    return (
        audit_pkg_list,
        report_dir,
        (host_volume, container_mountpoint, install_trace),
    )
