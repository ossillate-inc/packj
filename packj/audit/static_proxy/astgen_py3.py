...
class PythonDeclRefVisitor(ast.NodeVisitor):
    ...
    def generic_visit(self, node):
        ...
        if self.debug:
            if hasattr(node, 'lineno'):
                logging.warning(
                    f'visiting {type(node).__name__} node at line {node.lineno}'
                )
            else:
    ...
    def visit_Try(self, node):
        logging.debug(f'visiting Try node (line {node.lineno})')
        except_handlers = node.handlers
        if (
            len(except_handlers) == 1
            and len(except_handlers[0].body) == 1
            and isinstance(except_handlers[0].body[0], ast.Pass)
        ):
            node_details = {
                "Name": "",
                "File": self.infile,
                "StartLine": node.lineno,
                "EndLine": node.last_token.end[0],
            }
            self.all_declrefs["EmptyCatchClauses"].append(node_details)
        ast.NodeVisitor.generic_visit(self, node)
    ...
    def visit_Call(self, node):
            ...
            for fieldname, value in ast.iter_fields(node):
                ...
                if fieldname == 'func':
                    for f_fieldname, f_value in ast.iter_fields(value):
                        logging.info(
                            f'func fieldname {f_fieldname}, func value {f_value}'
                        )
                        if f_fieldname == 'id':
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            if isinstance(node.func.value, ast.Name):
                ...
                logging.warning(
                    f'node.func.value type ast.Subscript, fields: {list(ast.iter_fields(node.func.value))}'
                )
            else:
                base = self.asttok.get_text(node.func.value)
                logging.warning(
                    f'node.func.value type: {type(node.func.value)}, \
        ...
        # log stuff
        logging.warning(
            f'calling function {full_name} with args {args} at line {node.lineno}'
        )
        ...
        if (
            self.declrefs_filter_set is None
            or name_to_check in self.declrefs_filter_set
        ):
...
def py3_astgen(inpath, outfile, configpb, root=None, pkg_name=None, pkg_version=None):
    ...
    allfiles, infiles, bins, root = StaticAnalyzer._get_infiles(
        inpath=inpath, root=root, language=language
    )
    ...
    for infile in allfiles:
        ...
        try:
            ...
            logging.warning(
                f'Syntax error {str(se)} parsing file {infile} in Python3. Skipping!'
            )
            continue

        # mark the tree with tokens information
        try:
            asttok = asttokens.ASTTokens(
                source_text=all_source, tree=tree, filename=infile
            )
            visitor = PythonDeclRefVisitor(
                infile=infile, asttok=asttok, configpb=configpb
            )
            visitor.visit(tree)
            logging.warning(
                f'collected functions: {Counter(visitor.get_declrefs()).items()}'
            )

            filepb = StaticAnalyzer._get_filepb(infile, root)
            for base, name, args, source_text, source_range in visitor.get_declrefs():
                api_result = StaticAnalyzer._get_api_result(
                    base, name, args, source_text, source_range, filepb
                )
...
def parse_args(argv):
    ...
    parser.add_argument(
        "-b", "--root", dest="root", help="Path to the root of the source."
    )
    parser.add_argument(
        "-n",
        "--package_name",
